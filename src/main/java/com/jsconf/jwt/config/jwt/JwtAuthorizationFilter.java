package com.jsconf.jwt.config.jwt;
...
// 권한이나 인징으 필요한 특정 주소를 요청했을 때 위 필터를 무조건 타게 되어있음.
...
public class JwtAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    // 인증이나 권한이 필요한 주소요청이 있을 때 해당 필터를 타게 됨.ss
    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        String username =
                JWT.require(Algorithm.HMAC512("token")).build().verify(jwtToken).getClaim("username").asString();
